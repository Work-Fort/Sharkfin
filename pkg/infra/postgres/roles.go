...
func (s *Store) HasPermission(username, permission string) (bool, error) {
	...
	err := s.db.QueryRow(`
		...
		FROM users u
		JOIN role_permissions rp ON u.role = rp.role
		WHERE u.username = $1 AND rp.permission = $2
...
func (s *Store) SetUserRole(username, role string) error {
	res, err := s.db.Exec("UPDATE users SET role = $1 WHERE username = $2", role, username)
	if err != nil {
...
func (s *Store) SetUserType(username, userType string) error {
	res, err := s.db.Exec("UPDATE users SET type = $1 WHERE username = $2", userType, username)
	if err != nil {
