...
func (s *Store) CreateRole(name string) error {
	_, err := s.db.Exec("INSERT INTO roles (name) VALUES ($1)", name)
	if err != nil {
...
func (s *Store) DeleteRole(name string) error {
	res, err := s.db.Exec("DELETE FROM roles WHERE name = $1 AND built_in = FALSE", name)
	if err != nil {
...
func (s *Store) HasPermission(username, permission string) (bool, error) {
	...
	err := s.db.QueryRow(`
		...
		FROM identities i
		JOIN role_permissions rp ON i.role = rp.role
		WHERE i.username = $1 AND rp.permission = $2
...
func (s *Store) SetUserRole(username, role string) error {
	res, err := s.db.Exec("UPDATE identities SET role = $1 WHERE username = $2", role, username)
	if err != nil {
...
func (s *Store) SetUserType(username, userType string) error {
	res, err := s.db.Exec("UPDATE identities SET type = $1 WHERE username = $2", userType, username)
	if err != nil {
