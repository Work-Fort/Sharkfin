...
func (s *Store) OpenDM(userID, otherUserID int64, otherUsername string) (string, bool, error) {
	...
	callerUsername := ""
	if err := s.db.QueryRow("SELECT username FROM users WHERE id = ?", userID).Scan(&callerUsername); err != nil {
	...
	_, err = s.CreateChannel(dmName, false, []int64{userID, otherUserID}, "dm")
	if err != nil {
