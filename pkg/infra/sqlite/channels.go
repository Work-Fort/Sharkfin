...
func (s *Store) CreateChannel(name string, public bool, memberIDs []string, channelType string) (int64, error) {
	...
	for _, id := range memberIDs {
		if _, err := tx.Exec("INSERT INTO channel_members (channel_id, identity_id) VALUES (?, ?)", chID, id); err != nil {
...
func (s *Store) ListChannelsForUser(identityID string) ([]domain.ChannelWithMembership, error) {
	rows, err := s.db.Query(`
		SELECT DISTINCT c.id, c.name, c.public, c.type, c.created_at,
			CASE WHEN cm.identity_id IS NOT NULL THEN 1 ELSE 0 END AS member
		FROM channels c
		LEFT JOIN channel_members cm ON c.id = cm.channel_id AND cm.identity_id = ?
		WHERE c.type = 'channel' AND (c.public = 1 OR cm.identity_id IS NOT NULL)
		ORDER BY c.name
	`, identityID)
	if err != nil {
...
func (s *Store) AddChannelMember(channelID int64, identityID string) error {
	_, err := s.db.Exec(
		"INSERT INTO channel_members (channel_id, identity_id) VALUES (?, ?)",
...
func (s *Store) IsChannelMember(channelID int64, identityID string) (bool, error) {
	var count int
	err := s.db.QueryRow(
		"SELECT COUNT(*) FROM channel_members WHERE channel_id = ? AND identity_id = ?",
		channelID, identityID,
	).Scan(&count)
	if err != nil {
...
func (s *Store) ListDMsForUser(identityID string) ([]domain.DMInfo, error) {
	rows, err := s.db.Query(`
		...
		FROM channels c
		JOIN channel_members cm1 ON c.id = cm1.channel_id AND cm1.identity_id = ?
		JOIN channel_members cm2 ON c.id = cm2.channel_id AND cm2.identity_id != ?
...
func (s *Store) OpenDM(identityID, otherIdentityID string, otherUsername string) (string, bool, error) {
	...
	callerUsername := ""
	if err := s.db.QueryRow("SELECT username FROM identities WHERE id = ?", identityID).Scan(&callerUsername); err != nil {
	...
	_, err = s.CreateChannel(dmName, false, []string{identityID, otherIdentityID}, "dm")
	if err != nil {
